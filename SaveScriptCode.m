...
%% Write equations for YAW
if exist('H_YAW','var')
    fprintf(fid,'\n');
    fprintf(fid,'SH_YAW = zeros(%d,1);\n',numel(SH_YAW));
    for rowIndex = 1:numel(SH_YAW)
        string = char(SH_YAW(rowIndex,1));
        fprintf(fid,'SH_YAW(%d) = %s;\n',rowIndex,string);
    end
   [nRow,nCol] = size(H_YAW);
    fprintf(fid,'\n');
    fprintf(fid,'H_YAW = zeros(1,%d);\n',nCol);
    for rowIndex = 1:nRow
        for colIndex = 1:nCol
            string = char(H_YAW(rowIndex,colIndex));
            % don't write out a zero-assignment
            if ~strcmpi(string,'0')
                fprintf(fid,'H_YAW(1,%d) = %s;\n',colIndex,string);
            end
        end
    end
    fprintf(fid,'\n');
    fprintf(fid,'\n');
    fprintf(fid,'SK_YAW = zeros(%d,1);\n',numel(SK_YAW));
    for rowIndex = 1:numel(SK_YAW)
        string = char(SK_YAW(rowIndex,1));
        fprintf(fid,'SK_YAW(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
    
     fprintf(fid,'\n');
    fprintf(fid,'Kfusion_YAW = zeros(%d,1);\n',numel(K_YAW));
    for rowIndex = 1:numel(K_YAW)
        string = char(K_YAW(rowIndex,1));
        fprintf(fid,'Kfusion_YAW(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
end
%% Write equations for VEL
if exist('H_VEL','var')
    fprintf(fid,'\n');
    fprintf(fid,'Kfusion_VN = zeros(%d,1);\n',numel(K_VN));
    for rowIndex = 1:numel(K_VN)
        string = char(K_VN(rowIndex,1));
        fprintf(fid,'Kfusion_VN(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
    
    fprintf(fid,'\n');
    fprintf(fid,'Kfusion_VE = zeros(%d,1);\n',numel(K_VE));
    for rowIndex = 1:numel(K_VE)
        string = char(K_VE(rowIndex,1));
        fprintf(fid,'Kfusion_VE(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
    
    fprintf(fid,'\n');
    fprintf(fid,'Kfusion_VD = zeros(%d,1);\n',numel(K_VD));
    for rowIndex = 1:numel(K_VD)
        string = char(K_VD(rowIndex,1));
        fprintf(fid,'Kfusion_VD(%d) = %s;\n',rowIndex,string);
    end
    fprintf(fid,'\n');
end
